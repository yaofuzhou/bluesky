	...
	switch (vertex_scale_type) {
		case VERTEX_IS_SCREEN:
			...
			if (vertex_rotate_ownhdg) {
				vertex_out = ownrot * (zoom * flat_earth * position + SYMBOLSCALE * symbolrot * vertex);
			} else {
				vertex_out = ownrot * (zoom * flat_earth * position) + SYMBOLSCALE * symbolrot * vertex;
			}
		...
		case VERTEX_IS_METERS:
			...
			vertex_out = ownrot * zoom * (flat_earth * position + (degrees(vertex.yx * REARTH_INV)));
		...
		case VERTEX_IS_LATLON:
			...
			vertex_out = ownrot * zoom * flat_earth * (position + vertex.yx);
			texcoords_fs  = texcoords.ts;
			break;

		case VERTEX_IS_GLXY:
			if (vertex_rotate_ownhdg) {
				vertex_out = ownrot * vertex;
			} else {
				vertex_out = vertex;
			}
			texcoords_fs  = texcoords;
