...
const vec2[] texcoords_rect = vec2[6](vec2(0.0, 0.0), vec2(0.0, 1.0), vec2(1.0, 0.0), vec2(1.0, 0.0), vec2(0.0, 1.0), vec2(1.0, 1.0));
...
layout (location = 6) in float texdepth;
...
void main() {
    ...
    color_fs = color;
    texcoords_fs.st = texcoords_rect[gl_VertexID % 6];
    texcoords_fs.p  = texcoords.p - 30.0;
    ...
    vec2 vertex_out = vertex; //vcoords[gl_VertexID%6] * char_size + vec2(float(gl_VertexID/6) * char_size[0], 0.0);
    ...
    switch (vertex_scale_type) {
        case VERTEX_IS_SCREEN:
            ...
            if (vertex_rotate_ownhdg) {
                vertex_out    = ownrot * (zoom * flat_earth * position + SYMBOLSCALE * symbolrot * vertex_out);
            } else {
                vertex_out    = ownrot * (zoom * flat_earth * position) + SYMBOLSCALE * symbolrot * vertex_out;
            }
            break;
        ...
        case VERTEX_IS_GLXY:
            if (vertex_rotate_ownhdg) {
                vertex_out    = ownrot * vertex_out;
            }
            break;
    }

    gl_Position = vec4(vertex_out.x, vertex_out.y - 0.7, 0.0, 1.0);
