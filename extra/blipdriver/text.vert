...
void main() {
    ...
    texcoords_fs.p  = texcoords.p - 32.0;
    ...
    vec2 vertex   = vertex; //vcoords[gl_VertexID%6] * char_size + vec2(float(gl_VertexID/6) * char_size[0], 0.0);

    if (block_size[0] > 0) {
        texcoords_fs.p = texdepth - 32.0;
